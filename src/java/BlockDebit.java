    ...
    java.sql.Date ourJavaDateObject = new java.sql.Date(calendar.getTime().getTime());
            ResultSet rs;
             ...
             PreparedStatement ps=con.prepareStatement("select * from Account1 where acno=? and password=? ");
                 ...
                 String INSERT_RECORD = "insert into requesttable1(name,acno,type,dcno,date2)values(?,?,?,?,?)";
    
    PreparedStatement pstmt = con.prepareStatement(INSERT_RECORD);
    pstmt.setString(1,name);
    pstmt.setLong(2,y);
    pstmt.setString(3, "BLOCK DEBIT CARD REQUEST");
   pstmt.setLong(4,cno);
   // pstmt.setLong(5,0);
    //pstmt.setLong(6,0);
    ...
    String  sq2 = "SELECT email FROM Account1" +
      ...
      while(rs.next()){
         ...
         email1=rs.getString("email");
           ...
           RequestDispatcher rd1=request.getRequestDispatcher("./succdcblock.html");
                  ...
                  else
                  {
                        ...
                        RequestDispatcher rd2=request.getRequestDispatcher("./invalidpassdcblock.html");
           ...
           /* out.println("<!DOCTYPE html>");
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet BlockDebit</title>");            
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet BlockDebit at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");*/
        }
    }
