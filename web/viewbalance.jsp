<%-- 
    ...
    Created on : 25 Feb, 2016, 8:57:50 PM
    Author     : Avdesh Tyagi
--%>
...
<html>
 ...
 <%@ page import = "java.sql.*" %>
  ...
  //ResultSet rs = 
        ...
        //PreparedStatement ps=con.prepareStatement("select balance from Account1 where password=? ");
            PreparedStatement ps=con.prepareStatement("select balance from Account1 where acno=? ");
                  ps.setLong(1, acnu);
                  //ps.setString(2, z);
                   //ps.setString(3, br);
 ...
 <TABLE BORDER="1">
            ...
            <em><h1 style="color:blue;height:30px;  margin-left: 35px; width:400px;font-size:22pt;" >Account Number=<%= acnu %> </h1></em>
            <em><h1 style="color:blue;height:30px;  margin-left: 5px; width:400px;font-size:22pt;">Available balance=<%= rs.getLong(1) %> </h1></em>
            <% } %>
        </TABLE>
